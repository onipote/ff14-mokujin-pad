...
## refine: ジャッジメントポップアップを一定速度・デメリット下向きに変更
...
| ファイル | 内容 |
|---------|------|
| `styles/main.css` | ポップアップアニメーションのイージングを `ease-out` → `linear` に変更、MISSを下向きに落下させる `judgment-fall` キーフレームを追加 |

### 詳細

- `judgment-rise` の `ease-out` を `linear` に変更し、全ポップアップが一定速度で動くように統一
- MISS（デメリット）は新設の `judgment-fall` キーフレームで下方向（+48px）に落下
- GREAT / GOOD / bonus（メリット）は従来通り上方向（-48px）に上昇

---

## fix: GOODウィンドウ終端でのボタン押下がMISS扱いになるバグを修正
...
| ファイル | 内容 |
|---------|------|
| `src/game.js` | `_onTimeout()` でミス判定を1RAFフレーム遅延させ、入力ループが先に処理されるよう修正 |

### 詳細

**問題の構造**

ゲームパッドボタンの入力検出は `requestAnimationFrame` ループ（約16ms間隔）でポーリングしているのに対し、スロットのタイムアウト（MISSトリガー）は `setTimeout` で発火する。`setTimeout` コールバックはRAFフレームの間に割り込むため、以下の競合が発生していた：

1. プレイヤーがGOODウィンドウ終端（elapsedRatio≈1.99）でボタンを物理的に押す
2. `_timeoutId`（setTimeout）が先に発火 → `_onTimeout` → `state = 'feedback'`
3. 次のRAFフレームで入力ループが実行 → `_onInput` → `state !== 'showing'` で早期リターン
4. 押したのにMISS扱いになる

**修正**

`_onTimeout` 内でミス判定処理を `requestAnimationFrame` でラップし、1フレーム後に実行するよう変更した。RAFコールバックは登録順に実行されるため、入力ループのRAF（各フレーム末尾で登録済み）が必ず先に動き、ボタン押下があれば state が変わってミス判定がキャンセルされる。ボタン未入力の場合は state がそのまま `'showing'` なのでミス判定が通常通り実行される。

---

## fix: サブタイトル変更・スティックギミック失敗をMISS集計に反映
...
| ファイル | 内容 |
|---------|------|
| `index.html` | サブタイトルを「指酷のクロスホットバー」→「昇天のクロスホットバー」に変更 |
| `README.md` | タイトルの読みを「しっこく」→「昇天（しょうてん）」に変更 |
| `src/game.js` | `_onAoeHit()` でスティックギミック失敗時に `missCount` をインクリメント |

### 詳細

**サブタイトル変更**

サブタイトルを「指酷（しっこく）」から「昇天（しょうてん）」に変更。

**スティックギミック失敗をMISS集計へ反映**

左スティック（AoE回避失敗）・右スティック（宝箱未捕捉）はいずれも `aoe.js` 内で `onHit` コールバックを呼び出し、`game.js` の `_onAoeHit()` に到達する。これまでコンボリセットと時間ペナルティは発生していたが `missCount` は加算されていなかった。`_onAoeHit()` に `this.missCount++` を追加することで、リザルト画面のMISS表示にスティックギミック失敗も含まれるようになった。
